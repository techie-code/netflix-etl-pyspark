...
Spark's native Hadoop-based file commit protocol (`FileOutputCommitter`) fails on Windows without a correctly configured `winutils.exe`, a known environment issue. Output here is written via `.collect()` and Python's built-in `csv` module instead, which sidesteps the Hadoop commit path entirely for these small aggregated result sets. A reasonable approach when result sets are summary-sized, not appropriate for full-dataset writes at production scale.
